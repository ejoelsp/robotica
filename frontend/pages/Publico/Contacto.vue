  ...
  <section class="mx-auto max-w-7xl px-3 py-8 sm:px-6 sm:py-12">
    <div class="mb-8">
      ...
      <p class="mt-2 max-w-2xl text-sm text-slate-600">
        Integrantes responsables de la organización
    ...
    >
      Aun no hay integrantes activos del comité organizador para mostrar.
    </div>
    ...
    <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
      ...
      >
        <div class="flex items-start gap-3 sm:gap-4">
          <div class="h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-slate-100 sm:h-20 sm:w-20 sm:rounded-2xl">
            <img
            ...
            />
            <div v-else class="flex h-full w-full items-center justify-center text-lg font-bold text-slate-400 sm:text-xl">
              {{ integrante.nombres?.charAt(0) }}{{ integrante.apellidos?.charAt(0) }}
            </div>
          </div>

          <div class="min-w-0">
            <h2 class="break-words text-base font-bold text-slate-900 sm:text-lg">
