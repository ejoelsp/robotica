...
class StoreAsignacionJuezRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $juecesSeleccionados = $principales->merge($apoyos);

            $juecesDuplicados = DB::table('vinculaciones.asignaciones_juez_categoria')
                ->where('categoria_id', $categoriaId)
                ->whereIn('juez_user_id', $juecesSeleccionados->all())
                ->pluck('juez_user_id');

            if ($juecesDuplicados->isNotEmpty()) {
                $validator->errors()->add(
                    'jueces_apoyo_ids',
                    'Uno o mas jueces ya estan registrados en esta categoria.'
                );
            }

            $principalExistente = DB::table('vinculaciones.asignaciones_juez_categoria')
                ->where('categoria_id', $categoriaId)
                ->where('rol', 'principal')
                ->exists();
            ...
            if ($principalExistente) {
                $validator->errors()->add(
                    'juez_principal_ids',
                    'La categoria ya tiene jueces principales registrados.'
                );
            }
        });
    }
